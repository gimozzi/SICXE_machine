...
int command_chk(LinkedList1* pList){
	int comm_arg[17] = {0,0, 0,0, 0,0, 0,0, 9,9, 2,2, 3,3, 0, 1, 0};
	//각각 명령어의 argument 개수
...
void m_comm_edit(VM* vm, LinkedList1* pList){//edit
	int start, value;
	int hex, len, hi;
	int line, idx;
	char string[81];
...
void m_comm_fill(VM* vm, LinkedList1* pList){//fill
	int start, end, value;
	int line, idx;
	int i, k;
	int hi, hex, len;
...
void o_comm_mnemonic(LinkedList1* pList){//opcode mnemonic
	ListNode1* temp;
	int key;
	int i;
	char mnemonic[81];
	...
	display_mnemonic(key, mnemonic);
}

void o_comm_list(){		//opcode list
	int i;	
	//display
