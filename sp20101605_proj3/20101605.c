...
//	char** directives;
	...
	HT3* ESTAB;			//External Symbol TABle
	LinkedList8 *T8_List;		//bp list
	LinkedList9 *T9_List;		//T record
...
//	asm_directives(directives);
	...
	comm_arr = (char **)malloc(24 * sizeof(char*));
	for(i=0 ; i<24 ;i++){
	...
	////////////////////////////////////////////////////////////
	//prj3
	ESTAB = (HT3 *)malloc(ESTABSIZE * sizeof(HT3));
	...
	////////////////////////////////////////////////////////////
	T8_List = T8_createList();
	T9_List = T9_createList();
...
//						printf("여긴가?\n");
				...
				case 1:
					...
					switch(sw){
						...
						case 12://symbol
							...
							//T6 List 할당 해제
							break;
						case 13://progaddr address
							PROGADDR = l_comm_progaddr(T1_List);
							BP = PROGADDR;
							break;
						case 14://loader
							total_length = l_comm_loader(T1_List, ESTAB, PROGADDR, vmemory, T9_List);
							break;
						case 15://run
							BP = l_comm_run(reg, PROGADDR, vmemory, T8_List, hashtable, total_length, T9_List, &runrunrun, BP);
							break;
						case 16://bp
							l_comm_bp(T1_List, T8_List);
							break;
	...
	T7_removeElement(ESTAB);	//T7 List 할당 해제
	T8_removeElement2(T8_List);	//T8 list 할당 해제(headNode포함)
	T9_removeElement2(T9_List);	//T8 list 할당 해제(headNode포함)
